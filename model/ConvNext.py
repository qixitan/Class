...
class Block(nn.Module):
    r""" ConvNeXt Block. There are two equivalent implementations:
        ...
        """
    def __init__(self, inplanes, drop_path=0., layer_scale_init_value=1e-6):
        super(Block, self).__init__()
        self.act = nn.GELU()
        self.dwconv = nn.Conv2d(inplanes, inplanes, kernel_size=7, padding=3, groups=inplanes)
...
class ConvNeXt(nn.Module):
    def __init__(
            self, inplanes, num_classes=1000, depths=[3, 3, 9, 3], dims=[96, 182, 364, 728],
            drop_path_rate=0., layer_scale_init_value=1e-6, head_init_scale=1.,
    ):
        super().__init__()
        self.downsample_layers = nn.ModuleList()
        stem = nn.Sequential(
            nn.Conv2d(inplanes, dims[0], kernel_size=4, stride=4),
            LayerNorm(dims[0], eps=1e-6, data_format="channels_first"),
        )
        self.downsample_layers.append(stem)
        for i in range(3):
            downsamle_layer = nn.Sequential(
                LayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
                nn.Conv2d(dims[i], dims[i+1], kernel_size=2, stride=2),
            )
            self.downsample_layers.append(downsamle_layer)
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[
                    Block(inplanes=dims[i], drop_path=dp_rates[cur+j],
